...
a_scores = 0
sp_scores = scores.split(' ')
for grade in sp_scores:
  if int(grade) >= 90:
    a_scores += 1
print(a_scores)
...
acro = '.'.join(word[:2].upper() for word in sent.lower().split() if word not in stopwords)
print(acro)
...
for goods in inventory:
  name,pair,price = goods.split(',')
  print('The store has{}{}, each for{} USD.'.format(pair,name,price))
